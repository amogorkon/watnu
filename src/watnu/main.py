...
class TaskList(QtWidgets.QDialog, task_list.Ui_Dialog):
    def __init__(self):
        ...
        def task_list_doubleclicked(row, column):
            ...
            text, okPressed = QtWidgets.QInputDialog.getMultiLineText(
                self, "Bearbeite Beschreibung...", "Aufgabe beschreiben", f"{task.do}"
            )
...
"""
        ...
        def _():
            ...
            if not self.task.ilk is ILK.habit:
                total = x if (x := (self.time_spent + self.adjust_time_spent + self.ticks)) > 0 else 0
            else:
...
"""
        ...
        def _():
            ...
            self.hide()
            write_session(
                self.task.id, self.start_time, stop_time, finished=False, pause_time=self.pause_time
            )
            win_main.show()
            if self.win_list:
...
"""
            ...
            self._extracted_from_start_task_29(mb)
        query = submit_sql(
            """
...
"""
    ...
    # TODO Rename this here and in `start_task`
